...
freecad-cli 会**自动检测** FreeCAD 安装路径（macOS/Linux），大多数情况下无需手动配置。

**自动支持的系统：**
- macOS: `/Applications/FreeCAD.app/Contents/Resources`
- Linux: `/usr/lib/freecad-python3`

**如果自动检测失败**（手动安装到非标准路径），手动设置环境变量：
...
# 期望看到: "freecad_available": true
```

**已知限制：**
- Draft、Arch 模块需要 PySide6（Qt GUI 绑定），headless 模式下不可用
- FreeCAD 1.x 和 0.19.x 的部分 API 存在差异，CLI 已做兼容性处理
